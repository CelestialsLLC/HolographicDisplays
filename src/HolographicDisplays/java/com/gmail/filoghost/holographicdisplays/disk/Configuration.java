...
/**
 * Just a bunch of static varibles to hold the settings. Useful for fast access.
 */
public class Configuration {

    public static double spaceBetweenLines = 0;
    ...
    public static boolean lineLengthAnalysis;
    ...
    public static void load(Plugin plugin) {
        ...
        try {
            ...
            e.printStackTrace();
            plugin.getLogger()
                    .warning("I/O error while reading the configuration. Was the file in use?");
            return;
        } catch (Exception e) {
            e.printStackTrace();
        ...
        if (needsSave) {
            ...
            try {
                ...
                e.printStackTrace();
                plugin.getLogger()
                        .warning("I/O error while saving the configuration. Was the file in use?");
            }
        }
        ...
        imageSymbol =
                StringConverter.toReadableFormat(
                        config.getString(ConfigNode.IMAGES_SYMBOL.getPath()));
        transparencySymbol =
                StringConverter.toReadableFormat(
                        config.getString(ConfigNode.TRANSPARENCY_SPACE.getPath()));
        ...
        pingerOfflineMotd =
                StringConverter.toReadableFormat(
                        config.getString(ConfigNode.BUNGEE_PINGER_OFFLINE_MOTD.getPath()));
        pingerStatusOnline =
                StringConverter.toReadableFormat(
                        config.getString(ConfigNode.BUNGEE_PINGER_ONLINE_FORMAT.getPath()));
        pingerStatusOffline =
                StringConverter.toReadableFormat(
                        config.getString(ConfigNode.BUNGEE_PINGER_OFFLINE_FORMAT.getPath()));

        if (pingerTimeout <= 0) pingerTimeout = 100;
        else if (pingerTimeout >= 10000) pingerTimeout = 10000;

        pingerServers = Utils.newMap();

        if (pingerEnable) {
            for (String singleServer :
                    config.getStringList(ConfigNode.BUNGEE_PINGER_SERVERS.getPath())) {
                ...
                if (nameAndAddress.length < 2) {
                    plugin.getLogger()
                            .warning(
                                    "The server info \""
                                            + singleServer
                                            + "\" is not valid. There should be a name and an address, separated by a colon.");
                    continue;
                }
                ...
                if (address.contains(":")) {
                    ...
                    try {
                        ...
                        plugin.getLogger()
                                .warning(
                                        "Invalid port number in the server info \""
                                                + singleServer
                                                + "\".");
                        continue;
                    }
        ...
        String tempColor =
                config.getString(ConfigNode.TRANSPARENCY_COLOR.getPath())
        ...
        if (!foundColor) {
            ...
            plugin.getLogger()
                    .warning(
                            "You didn't set a valid chat color for transparency in the configuration, light gray (&7) will be used.");
        }

        try {
            timeFormat = new SimpleDateFormat(config.getString(ConfigNode.TIME_FORMAT.getPath()));
            timeFormat.setTimeZone(
                    TimeZone.getTimeZone(config.getString(ConfigNode.TIME_ZONE.getPath())));
        } catch (IllegalArgumentException ex) {
            timeFormat = new SimpleDateFormat("H:mm");
            plugin.getLogger()
                    .warning("Time format not valid in the configuration, using the default.");
        }

        if (bungeeRefreshSeconds < 1) {
            plugin.getLogger()
                    .warning(
                            "The minimum interval for pinging BungeeCord's servers is 1 second. It has been automatically set.");
        ...
        if (bungeeRefreshSeconds > 60) {
            plugin.getLogger()
                    .warning(
                            "The maximum interval for pinging BungeeCord's servers is 60 seconds. It has been automatically set.");
            bungeeRefreshSeconds = 60;
        }

        if (!config.getBoolean(ConfigNode.SPACE_BETWEEN_LINES.getPath())) spaceBetweenLines = 0.02;
        lineLengthAnalysis = config.getBoolean(ConfigNode.LINE_LENGTH_ANALYSIS.getPath());
    }
}
