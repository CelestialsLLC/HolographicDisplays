...
/**
 * This is for the ProtocolLib versions without the WrappedDataWatcher.WrappedDataWatcherObject
 * class.
 *
 * <p>These versions are only used for 1.7 and 1.8.
 */
public class ProtocolLibHookImpl implements ProtocolLibHook {
                            ...
                            public void onPacketSending(PacketEvent event) {
                                ...
                                if (packet.getType()
                                    ...
                                    WrappedDataWatcher dataWatcher =
                                            spawnEntityPacket.getMetadata();
                                    String customName =
                                            dataWatcher.getString(customNameWatcherIndex);
                                    ...
                                    if (customName.contains("{player}")
                                        ...
                                        WrappedDataWatcher dataWatcherClone =
                                                dataWatcher.deepClone();
                                        dataWatcherClone.setObject(
                                    ...
                                    if (objectId != ObjectTypes.ITEM_STACK
                                            && objectId != ObjectTypes.WITHER_SKULL
                                            && objectId != ObjectTypes.ARMOR_STAND) {
                                        return;
                                    }
                                    ...
                                    if (entity.getType() != EntityType.HORSE
                                            && !VersionUtils.isArmorstand(entity.getType())) {
                                        // Enough, only horses and armorstands are used with custom
                                        // names.
                                        return;
                                    }
                                    ...
                                    for (int i = 0; i < dataWatcherValues.size(); i++) {
                                        WrappedWatchableObject dataWatcherValue =
                                                dataWatcherValues.get(i);

                                        if (dataWatcherValue.getIndex() == customNameWatcherIndex
                                                && dataWatcherValue.getValue() != null) {

                                            Object customNameObject = dataWatcherValue.getValue();
                                            if (customNameObject == null
                                                    || customNameObject instanceof String
                                                    == false) {
                                                return;
                                            }
                                            ...
                                            if (customName.contains("{player}")
                                                ...
                                                entityMetadataPacket =
                                                        new WrapperPlayServerEntityMetadata(
                                                                packet.deepClone());
                                                List<WrappedWatchableObject> clonedList =
                                                        entityMetadataPacket.getEntityMetadata();
                                                WrappedWatchableObject clonedElement =
                                                        clonedList.get(i);
                                                clonedElement.setValue(
                                                        customName
                                                                .replace(
                                                                        "{player}",
                                                                        player.getName())
            ...
            if (line.isSpawned()) {
                ...
                if (line instanceof CraftTextLine) {
                    ...
                    if (textLine.isSpawned()) {
                        ...
                        AbstractPacket nameablePacket =
                                new WrapperPlayServerSpawnEntityLiving(
                                        textLine.getNmsNameble().getBukkitEntityNMS());
                        nameablePacket.sendPacket(player);

                        if (textLine.getNmsSkullVehicle() != null) {
                            AbstractPacket vehiclePacket =
                                    new WrapperPlayServerSpawnEntity(
                                            textLine.getNmsSkullVehicle().getBukkitEntityNMS(),
                                            ObjectTypes.WITHER_SKULL,
                                            0);
                            vehiclePacket.sendPacket(player);

                            WrapperPlayServerAttachEntity attachPacket =
                    ...
                    if (itemLine.isSpawned()) {
                        ...
                        if (MinecraftVersion.isGreaterEqualThan(MinecraftVersion.v1_8)) {
                            ...
                            vehiclePacket =
                                    new WrapperPlayServerSpawnEntityLiving(
                                            itemLine.getNmsVehicle().getBukkitEntityNMS());
                        } else {
                            vehiclePacket =
                                    new WrapperPlayServerSpawnEntity(
                                            itemLine.getNmsVehicle().getBukkitEntityNMS(),
    ...
    private WrappedWatchableObject createWrappedWatchableObject(int index, Object value) {
        try {
            if (wrappedWatchableObjectConstructor == null) {
                wrappedWatchableObjectConstructor =
                        WrappedWatchableObject.class.getConstructor(int.class, Object.class);
            }

            return (WrappedWatchableObject)
                    wrappedWatchableObjectConstructor.newInstance(index, value);
        } catch (Exception ex) {
            throw new IllegalStateException(
                    "Could not invoke WrappedWatchableObject constructor", ex);
        }
    }
    ...
    private boolean isHologramType(EntityType type) {
        return type == EntityType.HORSE
                ...
                || VersionUtils.isArmorstand(type); // To maintain backwards compatibility
    }
