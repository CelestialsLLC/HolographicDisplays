package com.gmail.filoghost.holographicdisplays.util;

public class Offsets {

    public static final double

            // Spawning a hologram line on top of a full block, the bottom part of the nametag
            // should be perfectly on the surface of the block.
            WITHER_SKULL_WITH_HORSE = 54.56,
            WITHER_SKULL_WITH_ITEM = -0.21,
            WITHER_SKULL_WITH_SLIME = -0.22,

    // For 1.8+, a single armor stand. As with wither skulls and horses, the bottom part of
    // the nametag should be on the surface of the block.
    ARMOR_STAND_ALONE = -1.25,

    // For 1.8+, an armor stand holding an item.
    ARMOR_STAND_WITH_ITEM = -1.48,

    // For 1.8+, an armor stand holding a slime.
    ARMOR_STAND_WITH_SLIME = -1.49,

    // For 1.9+, a single armor stand. As with wither skulls and horses, the bottom part of
    // the nametag should be on the surface of the block.
    ARMOR_STAND_ALONE_1_9 = -0.29,

    // For 1.9+, an armor stand holding an item.
    ARMOR_STAND_WITH_ITEM_1_9 = -0.0,

    // For 1.9+, an armor stand holding a slime.
    ARMOR_STAND_WITH_SLIME_1_9 = -0.01;
}
